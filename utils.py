...
def extract_result_multilabel(text, labels):
    """extract_result"""
    ret, cur_types = [], []
    if len(text) != len(labels):
        # 韩文回导致label 比 text要长
        labels = labels[:len(text)]
    for i, label in enumerate(labels):
        if not all(l == u"O" for l in label):
            for l in label:
                if l != u"O":
                    _type = l[2:]
                    if l.startswith(u"B-"):
                        if not cur_types:
                            ...
                            ret[-1].append({"start": i, "text": [text[i]], "type": _type})
                        cur_types.append(_type)
                    elif cur_types and all([_type != cur_type and _type != u"O" for cur_type in cur_types]):
                        if not cur_types:
                            ret.append([{"start": i, "text": [text[i]], "type": _type}])
                        else:
                            ret[-1].append({"start": i, "text": [text[i]], "type": _type})
                        ...
                        for idx, cur_type in enumerate(cur_types):
                            if cur_type == _type:
                                ret[-1][idx]["text"].append(text[i])
        else:
            cur_types = []
    return [r for res in ret for r in res]
