...
from utils import read_by_lines, write_by_lines, extract_result, extract_result_multilabel
...
def predict_data_process(trigger_file, role_file, enum_file, schema_file,
                         save_path, use_multilabel = False):
...
#         print('ori role_ret',role_ret)
#         if _id in sent_role_mapping.keys():
#             print('sent_role_mapping[_id]',sent_role_mapping[_id])
#             tmp = Merge(role_ret,sent_role_mapping[_id])
#             print('tmp',tmp)
#             sent_role_mapping[_id] = tmp
#             print('sent_role_mapping[_id]',sent_role_mapping[_id])
        if _id in sent_role_mapping.keys():
            dd = sent_role_mapping[_id]
            for i in dd.keys():
                if i in role_ret.keys():
                    role_ret[i].append(dd[i][0])
                    print(role_ret)
            sent_role_mapping[_id] = role_ret
            print('multi lable data id ', _id)
            
        else:
            sent_role_mapping[_id] = role_ret        
        
#         sent_role_mapping[_id] = role_ret
#         print('sent_role_mapping',sent_role_mapping)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--multilabel', action='store_true', help="multilabel classification use")
    args = parser.parse_args()
    predict_data_process(args.trigger_file, args.role_file, args.enum_file,
                         args.schema_file, args.save_path, args.multilabel)
